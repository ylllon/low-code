...
import { Data, LayoutThree, Lightning, Share, Bug, Logout } from '@icon-park/vue-next'
import { computed, defineComponent, h, ref } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { useTime } from '@/hooks/useTime'
import { useCoreStore } from '@/core/store'
import { webapp } from '@/core/store/webapp'
// import '@/hooks/calculator'
...
const envStore = ref({
  debug:false,
  toggle: () => {
    envStore.value.debug =!envStore.value.debug
  }
})
...
<!--        />-->
      ...
      <h1 class="app-name">Yllon {{time}}</h1>
    </div>
    ...
    <div class="app-setting-wrapper">
      <div class="common-btn debug-btn" @click="logout">
        <Logout />
        退出登录
      </div>
